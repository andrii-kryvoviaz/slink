...
class CookieStore<T> implements Readable<T> {
  constructor(private readonly cookieName: string, private readonly defaultValue?: T) {
  }

  public subscribe(run: Subscriber<T>, invalidate?: Invalidator<T>): Unsubscriber {
    run(cookie.get(this.cookieName, this.defaultValue as unknown as string) as T);
...
export function cookieReadableStore<T>(cookieName: string, defaultValue?: T): Readable<T> {
