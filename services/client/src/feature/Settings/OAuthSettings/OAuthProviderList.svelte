<script lang="ts">
  ...
  interface Props {
    ...
    footer?: Snippet<[OAuthProviderDetails[]]>;
  }

  let { providers, onEdit, footer }: Props = $props();
...
{:else}
  <div
    class="rounded-xl bg-muted-soft/50 dark:bg-muted-soft/30 border border-muted overflow-hidden divide-y divide-muted"
  >
    <SortableList
    ...
    {@render footer?.(listState.providers)}
  </div>
{/if}
