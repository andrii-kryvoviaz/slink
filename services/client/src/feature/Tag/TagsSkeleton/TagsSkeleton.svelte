<script lang="ts">
  ...
  const treeDepths = [0, 1, 0, 0, 1, 2, 1, 0, 1, 0];
</script>

{#if variant === 'tree'}
  <div class="flex flex-col {customClass}">
    {#each skeletonItems as _, index}
      {@const depth = treeDepths[index % treeDepths.length]}
      <div
        class="flex items-center gap-2 rounded-lg py-1.5 pr-1.5"
        style="padding-left: {depth * 1.25 + 0.25}rem; animation-delay: {index *
          75}ms"
      >
        ...
        <Skeleton
          width="{70 + (index % 4) * 26}px"
          height="12px"
          rounded="sm"
        />
        <div class="ml-auto">
          <Skeleton width="22px" height="16px" rounded="md" />
        </div>
      </div>
    {/each}
  </div>
