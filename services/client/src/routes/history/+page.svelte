  ...
  <div class="container mx-auto flex flex-col px-4 py-6 sm:px-6 max-w-6xl">
    {#if historyFeedState.isEmpty}
      <div class="mt-16 flex grow flex-col items-center justify-center text-center">
        <div class="mb-8 p-6 rounded-full bg-gray-100 dark:bg-gray-800/50">
          <Icon icon="ph:clock-clockwise" class="w-12 h-12 text-gray-400 dark:text-gray-500" />
        </div>
        <h2 class="text-xl font-medium text-gray-900 dark:text-white mb-2">
          No uploads yet
        </h2>
        <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm">
          Start uploading images to see your upload history and manage your files.
        </p>
    ...
    {#if historyFeedState.isLoading}
      <div class="mt-8 flex justify-center">
        <div class="flex items-center gap-3 px-4 py-3 rounded-full bg-white dark:bg-gray-800/50 border border-gray-200/50 dark:border-gray-700/50 shadow-sm">
          <div class="w-4 h-4 border-2 border-gray-200 dark:border-gray-700 border-t-indigo-500 dark:border-t-indigo-400 rounded-full animate-spin"></div>
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Loading history...</span>
        </div>
      </div>
    {/if}
