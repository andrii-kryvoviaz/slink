          ...
          {#snippet itemTemplate(image)}
            ...
            >
              <div class="relative">
                ...
                {#if page.data.globalSettings?.image?.enableLicensing && image.license}
                  <div class="absolute bottom-2 right-2">
                    <Tooltip variant="dark" size="sm" side="top" width="lg">
                      {#snippet trigger()}
                        <div
                          class="flex items-center gap-1 px-1.5 py-0.5 rounded bg-black/50 backdrop-blur-sm text-white/80 text-[10px] cursor-help"
                        >
                          <Icon icon="ph:copyright" class="w-3 h-3" />
                          <span>{image.license.title}</span>
                        </div>
                      {/snippet}
                      <div class="space-y-1">
